...
class CustomSalesOrder(SalesOrder):
	...
	def validate_purchase_orders(self):
		"""
		Validates that all order for any eCommerce must have a pourchase order number
		"""
		if self.fulfillment_method not in ['MFN', 'AFN', 'eBay', 'WooCommerce', 'Walmart']:
			return
		fulfillment_method = self.fulfillment_method	
		if not self.po_no:
			if fulfillment_method in ['MFN', 'AFN']:
				fulfillment_method = "Amazon"
			frappe.throw(
				_(f"Sales Order from {fulfillment_method} platforms must have a Purchase Order Number.")
			)
