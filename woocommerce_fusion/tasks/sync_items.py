...
def _coerce_wc_images(images_field):
    """Return a list[dict] of images from either a JSON string, a list, or None."""
    if not images_field:
        return []
    if isinstance(images_field, list):
        return images_field
    try:
        data = json.loads(images_field)
...
def sync_woocommerce_products_modified_since(date_time_from=None):
	"""
	Fetch products changed since the cursor, in small, bounded chunks.
	Advance the cursor as we go so progress persists across runs.
	"""
	...
	if not date_time_from:
		error_text = _("'Last Items Syncronisation Date' field on 'WooCommerce Integration Settings' is missing")
	...
	products = get_list_of_wc_products(
	...
	for p in products:
		try:
			run_item_sync(woocommerce_product=p, enqueue=True)
			total_enqueued += 1
			# WooProduct Doc has either .woocommerce_date_modified or .date_modified
			last_seen_modified = getattr(p, "woocommerce_date_modified", None) or getattr(p, "date_modified", None)
		except Exception:
			pass

		if (frappe.utils.now_datetime() - start_ts).total_seconds() > MAX_SECONDS:
			break

	# Only move the cursor if we made progress
	if last_seen_modified:
		wc_settings.reload()
		try:
			dt = get_datetime(last_seen_modified)
		except Exception:
			dt = get_datetime(date_time_from)
		new_cursor = (dt + timedelta(seconds=1)).strftime("%Y-%m-%d %H:%M:%S")
		wc_settings.wc_last_sync_date_items = new_cursor
		wc_settings.flags.ignore_mandatory = True
		wc_settings.save()
	else:
		# touch settings (optional telemetry; keeps same cursor)
		wc_settings.reload()
		wc_settings.flags.ignore_mandatory = True
		wc_settings.save()

	frappe.logger("woocommerce_sync").info({
		"msg": "sync_woocommerce_products_modified_since finished",
		"from": str(date_time_from),
		"enqueued": total_enqueued,
		"advanced_to": wc_settings.wc_last_sync_date_items
	})
...
class SynchroniseItem(SynchroniseWooCommerce):
	...
	def update_item(self, woocommerce_product: WooCommerceProduct, item: ERPNextItemToSync):
		...
		if wc_server.enable_image_sync:
			wc_product_images = _coerce_wc_images(woocommerce_product.images)
			if wc_product_images:
				first = wc_product_images[0]
				src = (first.get("src") if isinstance(first, dict) else None)
				if src and item.item.image != src:
					item.item.image = src
					item_dirty = True
	...
	def create_item(self, wc_product: WooCommerceProduct) -> None:
		...
		if wc_server.enable_image_sync:
			wc_product_images = _coerce_wc_images(wc_product.images)
			if wc_product_images:
				first = wc_product_images[0]
				src = (first.get("src") if isinstance(first, dict) else None)
				if src:
...
def get_list_of_wc_products(
	...
	*,
	...
	start_ts = frappe.utils.now_datetime()
	MAX_SECONDS = 540  # 9 minutes; caller uses 10-min cap
	...
	while page <= max_pages:
		...
		args = {
			...
			# sort by modified ascending so we can advance the cursor safely
			"orderby": "modified",
			"order": "asc" if order_asc else "desc",

			# pass-through to HTTP layer (your utils will keep timeout & query auth)
			"_fields": "id,modified,sku" if slim_fields else None,
			"modified_after": get_datetime(date_time_from).isoformat() if date_time_from else None,
			"timeout": timeout,
		}
