...
class SynchroniseSalesOrder(SynchroniseWooCommerce):
	...
	def create_sales_order(self, wc_order: WooCommerceOrder) -> None:
		...
		sinv.is_pos = 1
		mop = wc_order.payment_method_title
		if mop:
			sinv.set("payments", [])
			sinv.append(
				"payments",
				{
					# "mode_of_payment": new_sales_order.woocommerce_payment_method,
					"mode_of_payment": mop if mop == "PayPal" else "Authorize.net",
					"reference_no": wc_order.transaction_id,
					"amount": new_sales_order.grand_total,
				}
			)
