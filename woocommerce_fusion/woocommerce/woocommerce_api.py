...
class WooCommerceResource(Document):
		...
		for server in wc_servers:
			...
			wc_api_list.append(
				WooCommerceAPI(
					...
					domain=parse_domain_from_url(server.woocommerce_server_url).lower(),  # ← normalized
				)
			)
	...
	def load_from_db(self):
		...
		target_domain = (wc_server_domain or "").lower()

		# 1) Exact domain match
		self.current_wc_api = next((api for api in self.wc_api_list if api.domain == target_domain), None)

		# 2) Fallbacks: match by substring or by configured server name (in case older docs stored name)
		if not self.current_wc_api:
			self.current_wc_api = next(
				(api for api in self.wc_api_list
				if target_domain in (api.woocommerce_server_url or "").lower()
				or target_domain == (api.woocommerce_server or "").lower()),
				None
			)

		# 3) Still nothing? Fail clearly with context instead of AttributeError
		if not self.current_wc_api:
			known = ", ".join(sorted({api.domain for api in self.wc_api_list}))
			log_and_raise_error(
				error_text=(
					f"load_from_db could not resolve WooCommerce server for domain '{wc_server_domain}'. "
					f"Known domains: {known}. Record: {self.name}"
				)
			)
		...
		try:
			...
			log_and_raise_error(error_text=error_text)
